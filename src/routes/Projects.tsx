...
function languageBadge(lang: string) {
  const base =
    "inline-flex items-center gap-1.5 rounded px-1.5 py-0.5 text-[10px] font-medium border bg-fg/5 border-accent/20 text-fg/80";
...
function RepoTopics({ repo }: { repo: GitHubRepo }) {
  const topics = (repo.topics && repo.topics.length > 0 ? repo.topics : inferTopics(repo)).slice(
    0,
    8,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const withLangs =
          Array.isArray(repos) && "languages" in (repos[0] ?? {})
        ...
        if (!cancelled) {
          setItems(withLangs);
          try {
            sessionStorage.setItem(CACHE_KEY, JSON.stringify({ ts: now, items: withLangs }));
          } catch {}
        }
      } catch (e: unknown) {
